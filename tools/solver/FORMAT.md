...
### 4.5 EVの基準点規約(2026-07-05実証済み: postflop-solver rev 9d1509f の
`src/game/interpreter.rs` `expected_values_detail`のソース精読により確定)

`.bin`に格納するEVは`expected_values_detail(現在の手番プレイヤー)`の出力を
bb換算(チップ÷10)したもの。その意味論は:

1. **「この決断時点から先の期待獲得チップ」**(サンクコスト除外)。実装上は生の
   反実仮想値に `starting_pot×0.5 + node.amount + bias` が加算されており、
   この規約の下で**フォールドのEVは常にちょうど0.0**になる(「降りれば追加の
   損得なし」基準)。つまりfold列の0.0は正しい値であり、消費側での特別扱いは不要。
2. **normalized weightが0のコンボ(ボードとブロックしている・レンジ外)のEVは
   0.0で埋められる**が、これは無意味なプレースホルダ値。TS側の採点・表示・
   突合テストでは、頻度/レンジ重みが0のコンボのEVを比較や集計に使わないこと。
3. アプリ側の採点(EVロス)は同一ノード内のアクション間EV差分のみを使うため、
   基準点の取り方(定数オフセット)自体は結果に影響しない。
4. **相手のそのノードへの到達確率が0のコンボのEVも無意味**(2026-08-03追記)。
   規約2は「normalized weightが0」=レンジ外・ボードブロックのケースだが、それとは別に
   **normalized weightは正なのに、自分の手が相手の継続レンジ全体をブロックしていて
   相手の反実仮想到達確率が0になる**ケースがある。このとき`expected_values_detail`は
   `*v *= normalizer * (w_raw / w_normalized)` で0近傍の反実仮想値に巨大な係数
   (normalizer=num_combinations、フロップでは10^6オーダー)を掛けるため、
   `allocate_memory(true)`で有効化しているi16圧縮の量子化残差が増幅され、
   構造上ありえない大きさのEV(実測: 開始ポット+2×実効スタックを超える値)が出る。

   これは**データ生成の失敗ではなく、その手では到達しえないノードのEVが未定義(0/0)で
   あることの現れ**。該当ノードは実戦上その手番側の手では発生しないため、再生成しても
   同じ値が再現する。消費側(採点・表示・突合テスト)は規約2と同様にこれらを使わないこと。

   実測例: `srp_co_vs_bb/KcJc2c.bin`のノード`bet75-raise55-allin`。モノトーン
   (KcJc2c)でOOPのオールインレンジが21コンボすべてAc入り(ナッツフラッシュ)のため、
   IPがAcを持つと相手到達確率がちょうど0になり、`8cAc`のcallのEVが204.95bb
   (構造上の最大は114.60bb)になる。全17シナリオ×95フロップ=1615ファイル中
   この1件のみ。
...
## 7. ターンバンドルフォーマット(P15 S2)

フロップ解の特定経路(アクション履歴)からターンへ到達した局面について、**あり得る
49枚のターンカード全てを1ファイルにまとめて事前計算**したもの。パスは
`{scenarioId}/{flopId}/{pathId}.bin`(`pathId`はセクション3のnodeIdエンコード。
例: `check-bet33-call`)。配信先はHugging Face Datasets(Section 4の`.bin`と異なり
リポジトリには同梱しない)。

### 7.1 設計方針

**新しいバイナリ構造を極力増やさない**: 個々のターンカードの解は、Section 4で
定義済みの単体`.bin`ブロブ(4.1ヘッダ〜4.4データ本体)を**そのまま**49個生成し、
小さなバンドルヘッダ+索引で連結するだけとする。理由:
- Rust側は既存の`SolutionExport`→`write_binary()`をそのまま49回呼ぶだけで済む
  (新しいエンコードロジック不要)
- TS側は既存の`decodeSolutionFile()`(`src/gto/loader/binaryFormat.ts`)を
  索引で切り出したバイト列に対しそのまま再利用できる(新しいデコードロジック不要)
- 個々のブロブ内の`flop_card_ids`はフロップ3枚のまま(ターンカードは埋め込まない)。
  ターンカードの対応は**バンドル索引のキーのみ**が正とする

各ブロブのコンボ表・ノード表・データ本体は、そのターンカードでブロックされる
コンボを除外・再正規化した後の解(`solve_turn_subgame_from_flop_solution`の
既存出力)であり、Section 4.5のEV基準点規約もそのまま適用される。

### 7.2 バンドルヘッダ

| フィールド | 型 | 説明 |
|---|---|---|
| magic | `u8[4]` | ASCII `"GTOB"` |
| version | `u8` | 1 |
| scenarioId長/scenarioId | `u8`+`u8[]` | 識別用(各ブロブ内にも同一値が入るが、バンドル単位の識別を索引解析前に済ませるため重複保持する) |
| flop | `u8[3]` | rust形式card_id。識別用途で同様に重複保持 |
| pathId長/pathId | `u8`+`u8[]` | ASCII、Section 3のnodeIdエンコード(フロップ木上の経路)。**ブロブ内には存在しないバンドル固有情報** |
| turnCount | `u8` | 索引エントリ数(52-3=49が基本値。将来的な部分バンドルも許容するため決め打ちしない) |

### 7.3 索引表

`turnCount`件、**turnCardId昇順**で並べる:

| フィールド | 型 | 説明 |
|---|---|---|
| turnCardId | `u8` | rust形式card_id(0..51)。フロップ3枚とは重複しない |
| byteOffset | `u32` | 7.4本体セクション先頭からのバイトオフセット |
| byteLength | `u32` | そのブロブのバイト長 |

### 7.4 本体

索引の各エントリについて、`byteOffset`位置から`byteLength`バイトが、Section 4.1〜4.4と
完全に同一構造の単体`.bin`ブロブ(1ターンカード分の解)。

### 7.5 読み込み契約

TS側ローダは以下の2段構成とする:
1. バンドルヘッダ+索引をパースし、`Map<turnCardId, {offset, length}>`を返す
   (`decodeBundleIndex()`のような新関数、軽量・即時)
2. 実際に特定のターンカードが必要になった時点で、本体から該当バイト範囲を
   `decodeSolutionFile()`にそのまま渡す(1ターンカード分のみ都度デコード。
   49件を毎回全展開しない)

### 7.6 往復検証(★GTOドメイン最重要ルール)

新規追加分として、以下をテストで担保する:
- 実際に`solve_turn_subgame_from_flop_solution`で得た1ターン分の`freq`/`ev_bb`が、
  `write_binary()`→バンドル連結→TS側`decodeSolutionFile()`往復後も、量子化許容誤差
  (freq: ±1/255、ev: ±0.01bb)内で一致すること
- 索引のoffset/lengthが実際のブロブ境界と一致し、隣接ブロブを誤って読まないこと
  (境界にダミーバイトを仕込んだ意図的な破損テストなどで確認)
- 49件すべてのturnCardIdが一意で、フロップ3枚のcard_idを含まないこと

## 8. 変更履歴
- 2026-07-04: 初版作成(P3 Step 1)。actionTree.tsのオールイン再レイズ仕様変更(P2.5)を反映。
- 2026-08-02: Section 7(ターンバンドルフォーマット)追加(P15 S2)。
