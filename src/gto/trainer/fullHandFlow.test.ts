...
describe('FullHandController (実.binフィクスチャ+in-processファクトリによる統合テスト)', () => {
  ...
  it('P7-6b: ターンに到達したハンドが終了すると、ターンをバックグラウンドで精密リファインし決断を差し替える', async () => {
    // check-checkのみで進行するパッシブなラインで、フロップ→ターン→リバーの3街とも
    // ライブソルブ(=事前計算でないforLiveStreet呼び出し)が発生する。通しの通常運転で
    // ターン+リバーの2回、リファインでターンをもう1回=計3回forLiveStreetが呼ばれる
    // ことを確認する(オールイン等のライブソルブ非要求経路との対比)。
    const rng = fixedRng([1e-9])
    const waiter = createWaiter()
    let forLiveStreetCalls = 0
    const baseFactory = makeFactory()
    ...
    // in-processファクトリは同期的に解くため、最後のcheckでリバーのshowdownに到達した
    // 時点でリファインまで含めた一連のPromiseチェーンがまとめて解決し、この時点で
    // 既にforLiveStreetCalls===3(ターン+リバーのプレイ用+ターンのリファイン)に
    // 達していることが多い。最低でもプレイ用の2回は発生している。
    expect(forLiveStreetCalls).toBeGreaterThanOrEqual(2)

    // リファインが完了する(refining=falseに戻る)まで待つ。in-processファクトリは同期的に
    // 解くため、この時点で既に完了・emit済みのことが多いが、念のため待機ループにする
    // (finishOrRefineの設計上、refining=trueを観測したら必ずfalseのemitが後続する)。
    while (snap.phase === 'over' && snap.refining) {
      snap = await waiter.waitForPause()
    }
    expect(snap.refining).toBe(false)
    // waiter.updatesは(pendingフラグでの取りこぼしと無関係に)全emitを記録しているため、
    // refining=trueへの遷移が実際に起きたことをここで確認できる。
    expect(waiter.updates.some((u) => u.refining)).toBe(true)
    expect(forLiveStreetCalls).toBe(3) // +ターンのリファインソルブ
  ...
  it('P8-3: リファイン中のrefineProgressがprovider.progress()の値を反映し、完了後nullに戻る', async () => {
    // check-checkのみで進行し、ターン+リバーのプレイ用ソルブに続く3回目のforLiveStreet
    // 呼び出し(=リファイン)だけreadyの解決をゲートしつつprogress()を手動制御することで、
    ...
    const spyFactory: NodeProviderFactory = {
      ...
      forLiveStreet: (input: StreetSolveInput) => {
        ...
        const gated: StreetNodeProvider = { ...real, ready: gate.then(() => real.ready), progress: () => ({ fraction: refineProgressValue }) }
        return gated
      },
      dispose: () => baseFactory.dispose(),
    }
    ...
    // ターン+リバーの2回のプレイ用ソルブに続き、3回目(リファイン)の呼び出しがゲートされて
    // 未解決のまま止まっているはず。
  ...
  it('P7-6b: リファイン中にdispose()してもエラーにならない', async () => {
    const rng = fixedRng([1e-9])
    const waiter = createWaiter()
    const controller = new FullHandController({
      scenario,
    ...
    // リファイン完了を待たず、over到達直後に破棄する(実運用の「次のハンド」相当)。
