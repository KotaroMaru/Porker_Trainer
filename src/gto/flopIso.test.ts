...
describe('flopIso: 往復・全単射性', () => {
  ...
  it('収録95フロップ全件で、スート置換した別表記から元の収録フロップへ戻る(往復)', () => {
  ...
  it('95件の収録フロップは互いに異なる同型クラスに属する(代表フロップが同型で重複していない)', () => {
...
describe('flopIso: カバー率の固定化(回帰)', () => {
  it('全22100フロップ組み合わせに対する読み替え成功率は1420/22100(6.43%)で固定されている', () => {
    ...
    // 「往復」テスト(収録95フロップ全件)で正しさを検証済み。1420が正しい値。
    // 収録フロップを増減させた場合はこの回帰値を意図的に更新すること。
    expect(hit).toBe(1420)
  })
})
