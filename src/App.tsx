    ...
    <AnimatePresence>
      {settingsOpen && (
        ...
        >
          {isMobile && (
            <div>
              <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 6 }}>表示設定</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 13, color: 'var(--text)' }}>見積もりモード</span>
                  <Toggle label="" checked={estimateMode} onChange={toggleEstimateMode} />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 13, color: 'var(--text)' }}>型表示</span>
                  <Toggle label="" checked={showBotTypes} onChange={toggleShowBotTypes} />
                </div>
              </div>
            </div>
          )}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh' }}>
      <header style={{
        ...
        <span style={{ color: 'var(--gold)', fontWeight: 700, fontSize: 18, whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}>
          <SpadeIcon size={18} /> {isMobile ? 'PT' : 'Poker Trainer'}
        </span>
