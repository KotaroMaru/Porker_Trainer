import { getYokosawaTier, TIER_INFO, TIER_DISPLAY_ORDER } from '../advisor/yokosawa'
...
export function YokosawaRangeGrid({ highlightHand, cellSize = 30 }: Props) {
  ...
  return (
    <div style={{ display: 'inline-block' }}>
      ...
      {/* 凡例: 8 ティア(pink はBTNレイズにBBコール可の境界枠) */}
        ...
        {TIER_DISPLAY_ORDER.map(t => (
          <span key={t} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ color: 'var(--text-dim)', fontSize: 10.5 }}>
              {t === 'navy' ? '(8人/強)' : t === 'red' ? '(8人/弱)' : t === 'yellow' ? '(6〜7人)'
                : t === 'pink' ? '(境界: BTNレイズにBBコール可)' : t === 'gray' ? '(不参加)'
                : `(${TIER_INFO[t].maxBehind}人以下)`}
            </span>
          </span>
        ))}
        {highlightHand && (
